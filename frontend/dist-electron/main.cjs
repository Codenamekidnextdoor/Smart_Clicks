...
function createTray() {
    ...
    let iconPath = path_1.default.join(__dirname, '../public/tray-icon.png');
    try {
        tray = new electron_1.Tray(iconPath);
    }
    catch {
        try {
            iconPath = path_1.default.join(__dirname, '../public/icon.png');
            tray = new electron_1.Tray(iconPath);
        }
        catch (e) {
...
// ============================================================================
function createPopupWindow(text, screenX, screenY) {
    ...
    if (isDev) {
        ...
        popupWindow.loadFile(path_1.default.join(__dirname, '../dist/index.html'), { query: { mode: 'popup' } });
    }
...
function registerHotkeys() {
    ...
    const registered = electron_1.globalShortcut.register(hotkey, async () => {
 ...
 */
function captureTextFromBackend() {
    return new Promise((resolve) => {
        const req = http_1.default.request({ hostname: '127.0.0.1', port: 8000, path: '/api/capture-text', method: 'POST' }, (res) => {
            let data = '';
